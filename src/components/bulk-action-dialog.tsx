      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            ...
            {isSpellId
              ? currentDeck.spells.find((s) => s.id === selectedSpellType)
                  ?.name || "Selected"
              : (selectedSpellType ?? "").charAt(0).toUpperCase() +
                (selectedSpellType ?? "").slice(1)}{" "}
        ...
        {actionType === "edit" ? (
          ...
          <div className="space-y-4 py-4">
            ...
            <p className="text-sm text-muted-foreground">
              This will remove{" "}
              {isSpellId
                ? currentDeck.spells.filter(
                    (spell) => spell.id === selectedSpellType
                  ).length
                : currentDeck.spells.filter(
                    (spell) => spell.school === selectedSpellType
                  ).length}{" "}
              ...
              <Button
                ...
                onClick={() => {
                  if (selectedSpellType)
                    onDeleteSpellsByType(selectedSpellType, isSpellId);
                  onOpenChange(false);
                }}
              >
                Delete All
